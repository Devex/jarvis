...
from jarvis.jenkins.exceptions import UnknownJobError, ArgumentsFormatError, ArgumentsRequiredError
...
def build(message, job_name, args):
    ...
    try:
        ...
        build = job.run(args)
    except (UnknownJobError, ArgumentsFormatError, ArgumentsRequiredError) as e:
